...
-(void)isReady{
    ...
    [self appendMessageToResults:@"Device is ready for use. YOU ARE IN SANDBOX"];
...
- (void) deviceMessage:(NSString*)message{
    ...
    if ([message hasPrefix:@"BLE DEVICE FOUND: IDTECH"]) {
        // retrieve UUID
...
- (void) exampleUseJwtToRunPaymentTransaction:(NSString*)jwt {
    ...
    //Construct the url
    NSString *targetUrl = [NSString stringWithFormat:@"%@/rest/v2/mobile/transactions/sale", baseUrl];
...
- (void) exampleRequestReceipt:(NSString*)transactionId {
    ...
    if(txtReceiptEmailAddress.text == nil) {
        dict = @{@"id":transactionId,@"email-address":@"dhigginbotham@clearent.com"};
    } else {
...
- (IBAction) f_startAnyTransaction:(id)sender{
    ...
    RETURN_CODE rt = [clearentVP3300 device_startTransaction:amount amtOther:0 type:0 timeout:20 tags:nil forceOnline:false fallback:true];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
