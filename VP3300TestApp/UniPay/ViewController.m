...
-(void)isReady{
    ...
    [self appendMessageToResults:@"Device is ready for use. You can monitor the device's connection state by calling -> [clearentVP3300 isConnected].  YOU ARE IN SANDBOX"];
}

-(void) beepbeep {
    NSData* response;
    RETURN_CODE increaseStandByTimeRt = [[IDT_VP3300 sharedController] device_sendIDGCommand:0x01 subCommand:0x02 data:[IDTUtility hexToData:@"ff04ff00"] response:&response];
...
-(void)deviceDisconnected{
    ...
    [self appendMessageToResults:@"Device disconnected"];
...
- (void) deviceMessage:(NSString*)message{
    ...
    NSString *lookForFriendlyName = [NSString stringWithFormat:@"BLE DEVICE FOUND: %@",[friendlyName text]];
    NSLog(lookForFriendlyName);
   // [friendlyName text]
    if ([message hasPrefix:lookForFriendlyName]) {
   // if ([message hasPrefix:@"BLE DEVICE FOUND: IDTECH-VP3300"]) {
      //   retrieve UUID
        ...
        NSLog(@"UUID RETRIEVED %@", uuid);
    
   //      start a new search
        [self startSearchWithUUID:uuid];
    }
}
...
- (void)viewDidLoad
{
   ...
   clearentVP3300 = [[Clearent_VP3300 alloc] initWithConfig:self  clearentVP3300Configuration:clearentVP3300Config];
    ...
    //clearentVP3300 = [[Clearent_VP3300 alloc] init:self  clearentBaseUrl:baseUrl publicKey:publicKey];
  ...
  //  [clearentVP3300 setContactlessAutoConfiguration:false];
  //  [clearentVP3300 setContactless:false];
    
   
//    if([autoConfigure isOn]) {
...
//    }
    ...
    [friendlyName setText: @"IDTECH-VP3300-54998"];
...
//    NSDictionary* dict = @{@"amount":txtAmount.text,@"type":@"SALE",@"create-token":@"true"};
...
//    }
   ...
   // [self appendMessageToResults: @"Enabling Search for BLE"];
     ...
     //NSLog(@"UUID TO TRY %@", @"AFCD43DD-CB33-6935-B561-F8C749F1980D");
    
     [self appendMessageToResults:[friendlyName text]];
    NSUUID* val = nil;
   ...
   // if ([friendlyName text].length > 0) [clearentVP3300 device_setBLEFriendlyName:@"IDTECH"];
    NSString *lookForFriendlyName = [NSString stringWithFormat:@"%@",[friendlyName text]];
   // [clearentVP3300 startBluetoothScan:lookForFriendlyName];
...
//    }
    ...
    // DF7F is for collision
//    [tagsDictionary setObject:@"02" forKey:@"DF7F"];
//    NSData *tags = [IDTUtility encodeTags:tagsDictionary];
    ...
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
       // [self appendMessageToResults: @"Tap, insert, or swipe"];
        ...
        NSString * str = [NSString stringWithFormat:
                          @"ERROR: ID-\"%i\", message: %@.",
                          rt, [clearentVP3300 device_getResponseCodeString:rt]];
        [self appendMessageToResults:str];
        [self displayUpRet2: @"Start Transaction info" returnValue: rt];
    }
}
