...
-(void) displayUpRet2:(NSString*) operation returnValue: (RETURN_CODE)rt
{
    ...
    NSString * str = [NSString stringWithFormat:
                      @"%@ ERROR: ID-\"%i\", message: %@.",
                      operation, rt, [clearentPayments device_getResponseCodeString:rt]];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex{
    if ([alertView.title isEqualToString:@"Please Select"]) {
        if(buttonIndex == 1){
            ...
            //[[IDT_UniPayIII sharedController] emv_callbackResponseLCD:_lcdDisplayMode selection:(unsigned char)selectedValue];
            [clearentPayments emv_callbackResponseLCD:_lcdDisplayMode selection:(unsigned char)selectedValue];
    ...
    if (alertView == prompt_doConnection || alertView == prompt_doConnection_Low_Volume)
    {
        ...
        if (1 == buttonIndex) {
            ...
            [clearentPayments device_connectToAudioReader];
        }
    }
}
...
//CLEARENT: A public delegate implementation requiring you to provide a means of alerting you when the transaction token is successful. In this demo we've added code to use the JWT and immediately run a test payment transaction.
-(void) successfulTransactionToken:(NSString*) jsonString {
    ...
    [self exampleTransactionToClearentPayments:jwt];
}

- (void) errorTransactionToken:(NSString*)message{
    NSLog(@"ERROR FROM CLEARENT");
    [self appendMessageToResults:message];
    NSLog(@"%@",message);
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    NSLog(@"--viewDidAppear");
    //BOOL b = [[IDT_UniPayIII sharedController] isConnected];
    BOOL b = [clearentPayments isConnected];
    if(b==YES)
    {
        [self deviceConnected];
        [self appendMessageToResults:@"[[UniPay sharedController] Open Success]"];
    }
    else
    {
        [self deviceDisconnected];
        [self appendMessageToResults:@"[[UniPay sharedController] Open Failed]"];
    }
}
...
{    NSString *result;
    ...
    RETURN_CODE rt = [clearentPayments  device_getFirmwareVersion:&result];
...
{    NSString *result;
    ...
    RETURN_CODE rt = [clearentPayments  emv_getEMVL2Version:&result];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
- (IBAction) f_loadDefaultCAPK:(id)sender{
    ...
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] emv_setCAPKFile:CAPK];
    RETURN_CODE rt = [clearentPayments emv_setCAPKFile:CAPK];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
    ...
    //rt = [[IDT_UniPayIII sharedController] emv_setCAPKFile:CAPK];
    rt = [clearentPayments emv_setCAPKFile:CAPK];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
        [self appendMessageToResults: [NSString stringWithFormat:@"CAPK %@ Loaded",[CAPKString substringToIndex:12]]];
    }
...
- (IBAction) f_removeTermData:(id)sender{
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] emv_removeTerminalData];
    RETURN_CODE rt = [clearentPayments emv_removeTerminalData];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
- (IBAction) f_getSerialNumber:(id)sender
{
    ...
    RETURN_CODE rt = [clearentPayments config_getSerialNumber:&result];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
- (IBAction) f_passthroughON:(id)sender{
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] device_setPassThrough:true];
    RETURN_CODE rt = [clearentPayments device_setPassThrough:true];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
- (IBAction) f_passthroughOFF:(id)sender{
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] device_setPassThrough:false];
    RETURN_CODE rt = [clearentPayments device_setPassThrough:false];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
- (IBAction) f_DisableMSR:(id)sender{
    ...
    RETURN_CODE rt = [clearentPayments msr_cancelMSRSwipe];
    if(RETURN_CODE_DO_SUCCESS == rt){
...
- (IBAction) f_EnableMSR_waitData: (id)sender{
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] msr_startMSRSwipe];
    RETURN_CODE rt = [clearentPayments msr_startMSRSwipe];
    if(RETURN_CODE_DO_SUCCESS == rt){
...
- (IBAction) f_GetIccReaderStatus: (id)sender
{
    ...
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] icc_getICCReaderStatus:&response];
    RETURN_CODE rt = [clearentPayments icc_getICCReaderStatus:&response];
    if(RETURN_CODE_DO_SUCCESS != rt){
...
- (IBAction) f_IccPowerON: (id)sender{
    NSData* response;
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] icc_powerOnICC:&response];
    RETURN_CODE rt = [clearentPayments icc_powerOnICC:&response];
    if(RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) capkListing:(id)sender{
    NSArray *result;
    RETURN_CODE rt = 0;
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController]  ctls_retrieveCAPKList:&result];
    //RETURN_CODE rt = [clearentPayments ctls_retrieveCAPKList:&result];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) getTerminalFile:(id)sender{
    NSDictionary *result;
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController]  emv_retrieveTerminalData:&result];
    RETURN_CODE rt = [clearentPayments  emv_retrieveTerminalData:&result];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction)createTerminalFile:(id)sender{
    ...
    NSData* TLV = [IDTUtility hexToData:TLVstring];
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] emv_setTerminalData:[IDTUtility TLVtoDICT:TLV]];
    RETURN_CODE rt = [clearentPayments emv_setTerminalData:[IDTUtility TLVtoDICT:TLV]];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) createCRLFile:(id)sender{
    ...
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] emv_setCRLEntries:CRL];
    RETURN_CODE rt = [clearentPayments emv_setCRLEntries:CRL];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) getCAPKFile:(id)sender{
    NSData *result;
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController]  emv_retrieveCAPKFile:@"a000009999" index:@"e1" response:&result];
    RETURN_CODE rt = [clearentPayments  emv_retrieveCAPKFile:@"a000009999" index:@"e1" response:&result];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) getAIDFile:(id)sender{
    NSDictionary *result;
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController]  emv_retrieveApplicationData:@"a0000000031010" response:&result];
    RETURN_CODE rt = [clearentPayments  emv_retrieveApplicationData:@"a0000000031010" response:&result];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) removeCAPKFile:(id)sender{
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] emv_removeCAPK:@"a000009999" index:@"e1"];
    RETURN_CODE rt = [clearentPayments emv_removeCAPK:@"a000009999" index:@"e1"];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) removeCRL:(id)sender{
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] emv_removeCRLList];
    RETURN_CODE rt = [clearentPayments emv_removeCRLList];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) removeAIDFile:(id)sender{
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] emv_removeApplicationData:@"a0000000031010"];
    RETURN_CODE rt = [clearentPayments emv_removeApplicationData:@"a0000000031010"];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) getAIDFList:(id)sender{
    NSArray *result;
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController]  emv_retrieveAIDList:&result];
    RETURN_CODE rt = [clearentPayments  emv_retrieveAIDList:&result];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) getCAPKList:(id)sender{
    NSArray *result;
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController]  emv_retrieveCAPKList:&result];
    RETURN_CODE rt = [clearentPayments  emv_retrieveCAPKList:&result];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) getCRLList:(id)sender{
    ...
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController]  emv_retrieveCRLList:&result];
    RETURN_CODE rt = [clearentPayments  emv_retrieveCRLList:&result];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction) authEMV:(id)sender{
    ...
    RETURN_CODE rt = [clearentPayments emv_authenticateTransaction:nil];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
...
-(IBAction)startEMV:(id)sender{
    ...
    //RETURN_CODE rt = [[IDT_UniPayIII sharedController] emv_startTransaction:1.00 amtOther:0 type:0 timeout:60 tags:nil forceOnline:false fallback:true];
    RETURN_CODE rt = [clearentPayments emv_startTransaction:1.00 amtOther:0 type:0 timeout:60 tags:nil forceOnline:false fallback:true];
}
...
-(IBAction)completeEMV:(id)sender{
    ...
    //RETURN_CODE rt = [clearentPayments  emv_completeOnlineEMVTransaction:true hostResponseTags:[IDTUtility hexToData:@"8A023030"]];
    RETURN_CODE rt = 0;
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
